...
#include <PI/p4info.h>
#include <PI/pi.h>
#include <PI/target/pi_register_imp.h>
...
// with bmv2, typedef int64_t BmRegisterValue;
char * parse_bitstring(const BmRegisterValue &from) {

	char * bitstring = (char *) malloc(8 * sizeof(BmRegisterValue) + 1);
    memset(bitstring, 0, 8 * sizeof(BmRegisterValue) + 1);

    for (unsigned int i = 0; i < 8 * sizeof(BmRegisterValue); i++) {
        bitstring[i] = ((value & ((BmRegisterValue)0x1 << (63 - i))) != 0 ? '1' : '0');
    }
    return bitstring;
}

void convert_to_register_data(pi_register_data_t *to,
                             const BmRegisterValue &from) {
  // with bmv2, typedef int64_t BmRegisterValue;
  to->data_case = 1;

  char * bitstring = parse_bitstring(from);
  to->data_->bitstring_ = bitstring;
  free(bitstring);
}
...
pi_status_t _pi_register_read(pi_session_handle_t session_handle,
                             ...
                             pi_register_data_t *regsiter_data) {
  (void)session_handle;
  ...
  //TODO - pi_p4info_register_name_from_id()
  // std::string r_name(pi_p4info_register_name_from_id(p4info, register_id));

  std::string r_name("ingress.debuger");



  BmRegisterValue value;
  auto client = conn_mgr_client(pibmv2::conn_mgr_state, dev_tgt.dev_id);
  try {
  	value = client.c->bm_register_read(0, r_name, index);
  } catch(InvalidRegisterOperation &iro) {
  	const char *what =
        _RegisterOperationErrorCode_VALUES_TO_NAMES.find(iro.code)->second;
    std::cout << "Invalid register (" << r_name << ") operation ("
              << iro.code << "): " << what << std::endl;
    return static_cast<pi_status_t>(PI_STATUS_TARGET_ERROR + iro.code);
  }

  convert_to_register_data(regsiter_data, value);
